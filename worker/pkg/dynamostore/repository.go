...
type Repository struct {
	...
	now      func() time.Time
	// retentionLookup resolves the guild configuration Record reads message-retention
	// from. It defaults to the repository's own strict Load, but SetRetentionLookup lets
	// callers point it at a cached config.Provider instead, so Record's lookup shares a
...
// cached config.Provider to avoid a redundant DynamoDB read on every recorded message.
func (r *Repository) SetRetentionLookup(provider config.Provider) {
	r.retentionLookup = provider
}
...
// SetTier atomically assigns the subscription tier for one guild.
func (r *Repository) SetTier(ctx context.Context, guildID, actorID, tier string) (config.GuildConfig, error) {
...
func (r *Repository) Record(ctx context.Context, event *discordv1.DiscordMessageCreateEvent) error {
	...
	if event.GuildId != "" {
		lookup := r.Load
		if r.retentionLookup != nil {
			lookup = r.retentionLookup.Get
		}
		guildConfig, err := lookup(ctx, event.GuildId)
		if err != nil {
			if ctx.Err() != nil {
				return ctx.Err()
			}
...
type guildConfigItem struct {
	...
	Tier                  string   `dynamodbav:"tier,omitempty"`
...
func newGuildConfigItem(guildID, actorID string, value config.GuildConfig, updatedAt time.Time) guildConfigItem {
	...
	return guildConfigItem{
		...
		FallbackModelProfile: settings.FallbackModelProfile, AdminUserIDs: value.AdminUserIDs,
		Tier: value.Tier, Version: value.Version,
...
func (i guildConfigItem) config() config.GuildConfig {
	return config.GuildConfig{
		...
		AdminUserIDs: normalizedUserIDs(i.AdminUserIDs), Tier: i.Tier, Version: i.Version,
	}
}
