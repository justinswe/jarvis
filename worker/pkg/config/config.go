...
// tierNamePattern bounds subscription tier names to a safe, key-friendly shape.
var tierNamePattern = regexp.MustCompile(`^[a-z0-9][a-z0-9-]{0,31}$`)

// ValidateTier checks the shape of a subscription tier name, allowing the empty default.
//
// Tier membership is deliberately not checked here: the set of defined tiers is a
// deployment concern, and a guild assigned a tier that was later removed must keep
// working rather than failing every request.
func ValidateTier(tier string) error {
	if tier == "" || tierNamePattern.MatchString(tier) {
		return nil
	}
	return errors.New("tier must be lowercase alphanumeric with hyphens, at most 32 characters")
}
