...
// UsageReport is per-model token accounting for one generation.
...
// UsageRecorder receives per-model token accounting. Implementations must not block.
type UsageRecorder interface {
	RecordUsage(UsageReport)
}
