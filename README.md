...
A child's stderr is not passed through. nats-server writes every line it produces there, start-up notices included, so a collector that reads severity from the stream rather than the line reports a healthy broker as thousands of errors. The supervisor reads the `[INF]`/`[WRN]`/`[ERR]` tag instead and re-emits the line through its own logger at that level, tagged with the child it came from; an untagged line is a panic or a start-up failure and stays an error. Set `LOG_FORMAT=json` to have all four processes write one JSON object per line keyed `severity`, `message`, and `timestamp`.
...
Jarvis searches through Serper, Firecrawl, or Tavily when web search is enabled for a server. Serper must be first whenever it is configured. Explicit research, current facts, and actionable price, availability, safety, or recommendation questions require Search. A request may issue the original query and one refined query; each calls the first provider and may make one recovery call after an error or a response without usable sources. Results and diagnostics accumulate across both queries. Recovery uses the second configured provider, or retries the only provider once, and never starts after the request is canceled or its deadline expires. Runtime-only questions use the runtime-context tool without leaking into public web search, and image turns search only when the user asks for research or identification.
...
Within one worker instance, overlapping requests in the same Discord thread run FIFO. Every request gets its turn, and the next generation starts after the preceding reply has been recorded so it can use that answer as structured assistant history. Existing context-window and rune-budget settings still apply. Separate threads remain concurrent; deployments with multiple worker replicas need an ordering key or distributed queue to guarantee the same order across replicas.
...
With `STORE_DRIVER=postgres` (pgvector required) and `--memory-enabled` (default), memory is scoped to the current Discord channel or thread and character. Assistant-mode extraction sees only user-authored statements and stores explicit stable facts with their Discord subject ID; short-lived decisions expire after seven days, and assistant memories are never merged into unattributed digests. Roleplay mode may also retain rolling story summaries and plot state. Retrieval excludes quarantined and expired rows. The migration quarantines every legacy unpinned auto-memory instead of trusting old bot-authored or server-wide records; pinned records remain reviewable. Without an embedding profile, retrieval degrades to pinned + full-text. The memory book is a tool surface: `list_memories`, `pin_memory` (pinning clears expiry), `edit_memory`, `delete_memory`, `add_memory`. Background extraction is metered against guild tiers like any other generation.
...
| `STORE_SWEEP_INTERVAL`           | No                         | How often expired messages, request outcomes, short-lived memories, and lapsed reply claims are deleted; defaults to `1h`.                                               |
...
| `MEMORY_ENABLED`                 | No                         | Channel/thread-scoped assistant and character memory; defaults to `true` and activates only under `STORE_DRIVER=postgres` with pgvector available.                        |
