...
Server behavior is resolved for every targeted request through the configuration provider in `internal/config`. DynamoDB integration is disabled by default, in which case the worker uses validated hard-coded settings and Discord REST prompt history. Enable it with `DYNAMODB_ENABLED=true`; the worker then records every valid incoming MessageCreate event, reads conversation history from DynamoDB, searches the current channel's stored history, and allows authorized server administrators to manage per-server settings through model tools. Database configuration and initial authentication must succeed before the worker starts. Later request-time data failures are fail-open: message processing continues with defaults or explicitly partial context. See [docs/dynamodb.md](docs/dynamodb.md) for the table contract, search behavior, authentication modes, access model, and provisioning requirements.
