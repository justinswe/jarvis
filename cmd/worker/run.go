...
func runWorker(parent context.Context, cfg workerConfig) error {
	...
	if cfg.dynamodbEnabled {
		...
		if loadErr != nil {
			app.L().Warn("DynamoDB initialization failed; database integration disabled", zap.Error(loadErr))
		} else {
			...
			if repositoryErr != nil {
				app.L().Warn("DynamoDB repository initialization failed; database integration disabled", zap.Error(repositoryErr))
			} else {
